...
static void I386OP(bt_rm32_r32)()       // Opcode 0x0f a3
{
	...
	if( modrm >= 0xc0 ) {
		...
		UINT32 bit = LOAD_REG32(modrm);
		ea += 4*(bit/32);
		ea = i386_translate(segment,(m_address_size)?ea:(ea&0xffff),0,4);
		bit %= 32;
...
static void I386OP(btc_rm32_r32)()      // Opcode 0x0f bb
{
	...
	if( modrm >= 0xc0 ) {
		...
		UINT32 bit = LOAD_REG32(modrm);
		ea += 4*(bit/32);
		ea = i386_translate(segment,(m_address_size)?ea:(ea&0xffff),1,4);
		bit %= 32;
		UINT32 dst = READ32(ea);
...
static void I386OP(cmpsd)()             // Opcode 0xa7
{
	...
	ead = i386_translate(ES, m_address_size ? REG32(EDI) : REG16(DI), 0, 4 );
	src = READ32(eas);
	dst = READ32(ead);
	SUB32(src,dst);
	BUMP_SI(4);
		...
		if(now_debugging) {
			for(int i = 0; i < MAX_BREAK_POINTS; i++) {
				if(int_break_point.table[i].status == 1 && int_break_point.table[i].int_num == msdos_int_num) {
					if((int_break_point.table[i].ah == I386_AH || int_break_point.table[i].ah_registered == 0) &&
...
{
	UINT32 eas, src, dst;
	eas = i386_translate(ES, m_address_size ? REG32(EDI) : REG16(DI), 0, 4 );
	src = READ32(eas);
	dst = REG32(EAX);
	SUB32(dst, src);
	BUMP_DI(4);
...
{
	UINT32 eas = i386_translate(ES, m_address_size ? REG32(EDI) : REG16(DI), 1, 4 );
...
static bool I386OP(load_far_pointer32)(int s)
{
	UINT8 modrm = FETCH();
	UINT16 selector;
	bool fault = false;
