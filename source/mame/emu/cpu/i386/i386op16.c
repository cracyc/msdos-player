...
static void I386OP(bt_rm16_r16)()       // Opcode 0x0f a3
{
	...
	if( modrm >= 0xc0 ) {
		...
		UINT16 bit = LOAD_REG16(modrm);
		ea += 2*(bit/16);
		ea = i386_translate(segment,(m_address_size)?ea:(ea&0xffff),0,2);
		bit %= 16;
...
static void I386OP(btc_rm16_r16)()      // Opcode 0x0f bb
{
	...
	if( modrm >= 0xc0 ) {
		...
		UINT16 bit = LOAD_REG16(modrm);
		ea += 2*(bit/16);
		ea = i386_translate(segment,(m_address_size)?ea:(ea&0xffff),1,2);
		bit %= 16;
		UINT16 dst = READ16(ea);
...
static void I386OP(cmpsw)()             // Opcode 0xa7
{
	...
	ead = i386_translate(ES, m_address_size ? REG32(EDI) : REG16(DI), 0, 2 );
	src = READ16(eas);
	dst = READ16(ead);
	SUB16(src,dst);
	BUMP_SI(2);
...
static void I386OP(enter16)()           // Opcode 0xc8
{
	...
	if(level > 0)
	{
		for(x=1;x<=level-1;x++)
		{
			...
			PUSH16(READ16(i386_translate(SS, addr, 0, 2)));
		}
		PUSH16(frameptr);
	}
	REG16(BP) = frameptr;
...
static void I386OP(scasw)()             // Opcode 0xaf
{
	UINT32 eas;
	UINT16 src, dst;
	eas = i386_translate(ES, m_address_size ? REG32(EDI) : REG16(DI), 0, 2 );
	src = READ16(eas);
	dst = REG16(AX);
	SUB16(dst, src);
	BUMP_DI(2);
